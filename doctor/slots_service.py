...
def generate_slots_for_doctors(
    ...
    # 🕓 Round boundaries to nearest 5-minute or alignment boundary
    target_dt_start = round_up_to_nearest_five(target_dt_start)
    target_dt_end = round_down_to_nearest_five(target_dt_end)

    # ✅ Also round the min_allowed_start if provided
    if min_allowed_start:
        min_allowed_start = _align_to_boundary(min_allowed_start, alignment_minutes)
        min_allowed_start = round_up_to_nearest_five(min_allowed_start)
    ...
    for doctor in doctors_with_data:
        ...
        for avail_start, avail_end in availabilities:
            ...
            for block_start, block_end in free_blocks:
                # Align the block start properly before slot creation
                current_time = _align_to_boundary(block_start, alignment_minutes)
                current_time = round_up_to_nearest_five(current_time)
                ...
                if current_time < block_start:
                    ...
                    current_time = _align_to_boundary(block_start + alignment_delta, alignment_minutes)
                    current_time = round_up_to_nearest_five(current_time)
                ...
                # Generate slots in this block (pre-aligned)
                ...
                while current_time + session_duration_td <= block_end:
                    ...
                    # Always step in aligned intervals
                    current_time += alignment_delta
                    current_time = _align_to_boundary(current_time, alignment_minutes)
                    current_time = round_up_to_nearest_five(current_time)
...
#     # print( "\n\n\n\nin generate slot for doctors function line 435 datetime start:" ,target_dt_start ,"datetimeend", target_dt_end,"\n\n\n\n")
#     target_dt_start = round_up_to_nearest_five(target_dt_start)
#     target_dt_end = round_down_to_nearest_five(target_dt_end)
#     # print( "\n\n\n\nin generate slot for doctors function line 438 after rounding datetime start:" ,target_dt_start ,"datetimeend", target_dt_end,"\n\n\n\n")
...
def _generate_slot_response(
    ...
    print( "\n\n\n\nin generate slot response function datetime start:" ,date_time_start ,"datetimeend", date_time_end,"\n\n\n\n")
...
# def align_floor(dt: datetime, alignment_minutes: int) -> datetime:
#     """
#     Align datetime down (floor) to nearest alignment boundary.
#     Returns a timezone-aware datetime with seconds and microseconds cleared.
#     """
#     if alignment_minutes <= 0:
#         return dt.replace(second=0, microsecond=0)
#     # Work in total minutes since midnight for the dt's timezone
#     total_minutes = dt.hour * 60 + dt.minute
#     floored = (total_minutes // alignment_minutes) * alignment_minutes
#     hours = floored // 60
#     minutes = floored % 60
#     return dt.replace(hour=hours, minute=minutes, second=0, microsecond=0)


# def align_ceil(dt: datetime, alignment_minutes: int) -> datetime:
#     """
#     Align datetime up (ceil) to the nearest alignment boundary.
#     If dt is exactly on boundary, returns dt with seconds/microseconds zeroed.
#     """
#     if alignment_minutes <= 0:
#         return dt.replace(second=0, microsecond=0)
#     total_minutes = dt.hour * 60 + dt.minute
#     if total_minutes % alignment_minutes == 0 and dt.second == 0 and dt.microsecond == 0:
#         # already exactly aligned
#         return dt.replace(second=0, microsecond=0)
#     ceiled = ((total_minutes + alignment_minutes - 1) // alignment_minutes) * alignment_minutes
#     hours = ceiled // 60
#     minutes = ceiled % 60
#     # Handle overflow beyond 24h: shift by days
#     extra_days = hours // 24
#     hours = hours % 24
#     new_dt = dt.replace(hour=hours, minute=minutes, second=0, microsecond=0)
#     if extra_days:
#         new_dt = new_dt + timedelta(days=extra_days)
#     return new_dt
...
#     for block_start, block_end in free_blocks:
#         # Use ceil: first slot must be >= block_start and aligned
#         aligned_start = align_ceil(block_start, alignment_minutes)
        
#         # Generate slots within this block (stop if slot + required > block_end)
#         while aligned_start + total_required <= block_end:
#             slot = (aligned_start, aligned_start + duration_td)
#             clean_slots.append(slot)
#             aligned_start = aligned_start + duration_td + break_td
#             # no extra alignment needed for subsequent steps because we step by exact increments

#     return clean_slots


# def align_datetime_to_minutes(dt: datetime, alignment_minutes: int) -> datetime:
#     """Deprecated helper kept for compatibility — uses align_ceil semantics."""
#     return align_ceil(dt, alignment_minutes)
...
#     # FIX: Use round_down for target_dt_start to avoid accidentally moving the start forward
#     #      (we will still respect min_allowed_start explicitly below).
#     # Rationale: rounding UP on the target window plus later ceil-align can push the first aligned slot
#     # beyond available block boundaries; using round_down prevents that while min_allowed_start
#     # ensures we never return past timestamps.
#     target_dt_start = round_down_to_nearest_five(target_dt_start)  # FIX: changed from round_up_to_nearest_five
#     target_dt_end = round_down_to_nearest_five(target_dt_end)

#     # Prepare session and step deltas
#     session_duration_td = timedelta(minutes=session_duration)
#     alignment_delta = timedelta(minutes=alignment_minutes)

#     # Align min_allowed_start conservatively (ceil): earliest allowed slot boundary >= min_allowed_start
#     if min_allowed_start:
#         # zero seconds/microseconds then ceil to alignment boundary
#         min_allowed_start = min_allowed_start.replace(second=0, microsecond=0)
#         min_allowed_start = align_ceil(min_allowed_start, alignment_minutes)
#         # Ensure min_allowed_start is not earlier than target_dt_start
#         if min_allowed_start < target_dt_start:
#             min_allowed_start = target_dt_start
#         # Also ensure min_allowed_start is not after the target_dt_end
#         if min_allowed_start >= target_dt_end:
#             # no slots can be created in this range at all
#             return {}
...
#         # Generate slots for each availability period
#         for avail_start, avail_end in availabilities:
#             # Constrain to target range and MIN allowed start immediately
#             # FIX: include min_allowed_start in constrained_start so rounding/alignment
#             #      decisions operate on a block already trimmed to min allowed start.
#             if min_allowed_start:
#                 constrained_start = max(avail_start, target_dt_start, min_allowed_start)
#             else:
#                 constrained_start = max(avail_start, target_dt_start)
...
#             # Generate aligned slots
#             for block_start, block_end in free_blocks:
#                 # Use the already-clamped block_start (it is >= min_allowed_start and >= target_dt_start)
#                 # Find the first aligned start that is >= block_start (ceil)
#                 current_time = align_ceil(block_start, alignment_minutes)

#                 logger.debug(f"{RED}Doctor {doctor.doctor_profile_id} initial aligned block start candidate: {current_time} to block end {block_end}{RESET}")

#                 # If the ceil pushed beyond block_end, skip
#                 if current_time >= block_end:
#                     logger.debug(f"{RED}Doctor {doctor.doctor_profile_id} first aligned time {current_time} >= block_end {block_end}, skipping block{RESET}")
#                     continue

#                 # Generate slots in this block (pre-aligned and stepped by alignment_delta)
#                 logger.debug(f"{BLUE}Doctor {doctor.doctor_profile_id} generating slots starting at {current_time} until {block_end}{RESET}")

#                 while current_time + session_duration_td <= block_end:
#                     # Add slot (current_time is already >= min_allowed_start if min was present)
#                     slot_key = (current_time, current_time + session_duration_td)
#                     temp_slot_pool[slot_key].add(doctor.doctor_profile_id)

#                     # step forward exactly by alignment_delta
#                     current_time = current_time + alignment_delta
...
# def _align_to_boundary(dt: datetime, alignment_minutes: int) -> datetime:
#     """Align datetime to nearest boundary without skipping valid start."""
#     # Keep for compatibility; implement as floor alignment
#     if alignment_minutes <= 0:
#         return dt
#     return align_floor(dt.replace(second=0, microsecond=0), alignment_minutes)
